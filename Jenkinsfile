pipeline {
    ...
    stages {
        ...
        stage("git login"){
            steps{
                git 'https://github.com/dineshdheera/jenkins-docker.git'
            }
        }
        ...
        sh 'docker image tag $JOB_NAME:v1.$BUILD_ID dinnesh/$JOB_NAME:v1.$BUILD_ID'
        sh 'docker image tag $JOB_NAME:v1.$BUILD_ID dinnesh/$JOB_NAME:latest'
             }
         }
         stage("push Image: DOCKERHUB"){
             steps{
                 withCredentials([string(credentialsId: 'docker_pwd', variable: 'docker_passwd')]) {
                 sh "docker login -u dinnesh -p ${docker_passwd}"
                sh 'docker image push dinnesh/$JOB_NAME:v1.$BUILD_ID'
                sh 'docker image push dinnesh/$JOB_NAME:latest'
              ...
              sh 'docker image rm $JOB_NAME:v1.$BUILD_ID dinnesh/$JOB_NAME:v1.$BUILD_ID dinnesh/$JOB_NAME:latest'
              }
             }
         }
           stage("Docker Container Deployment"){
                steps {
        // def docker_run = 'docker run -p 9000:80 -d --name scripted-pipeline-demo dinnesh/scripted-pipeline-demo:latest'
        //def docker_rmv_container = 'docker run -p 9000:80 -d --name scripted-pipeline-demo vikashashoke/scripted-pipeline-demo:latest'
        //def docker_rmi = 'docker rmi -f dinnesh/scripted-pipeline-demo'
    ...
    sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.32.215 docker rmi -f dinnesh/scripted-pipeline-demo"
    sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.32.215 docker run -p 9000:80 -d --name scripted-pipeline-demo dinnesh/scripted-pipeline-demo:latest"
    ...
    //But still it'll not update because latest image is already in server it's not going to dockerHub to pull latest commmit so need to remove images also
        ...
        /* stage("Building Docker Image"){
            steps{
               ...
               //sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.92.83.182'
               //sh 'cd /opt/ansible'
               sh 'docker image build -t $JOB_NAME:v1.$BUILD_ID .'
               sh 'docker image tag $JOB_NAME:v1.$BUILD_ID dinnesh/$JOB_NAME:v1.$BUILD_ID'
               sh 'docker image tag $JOB_NAME:v1.$BUILD_ID dinnesh/$JOB_NAME:latest'
